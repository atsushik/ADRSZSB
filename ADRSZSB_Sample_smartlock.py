...
if __name__ == "__main__":
        ...
        duty_max = svo.Get_servo_duty_max("1")
        duty_min = svo.Get_servo_duty_min("1")
        print (duty_min , "->", duty_max)

        duty_mid = (duty_max - duty_min)/2 + duty_min

        svo.Set_servo_duty(1,duty_mid)
        svo.Set_servo_Write()

        while True:
                print()
                cmd = input("[L]ock/[U]nlock:")

                if cmd == "l" or cmd == "L":
                        svo.Set_servo_duty("1",duty_min)
                        svo.Set_servo_Write()
                        time.sleep(0.4)
                        svo.Set_servo_duty("1",duty_mid)
                        svo.Set_servo_Write()
